...
MicroModal.init({
    onShow: function (modal) {
        ...
        var newurl = window.location.protocol + "//" + window.location.host + window.location.pathname + '?show_action=' + modal.id;
...
(function () {
    ...
    if (homeHeroSlideshowEl) {
        var homeHeroSlideshow = Peppermint(homeHeroSlideshowEl, {
            ...
            dots: true,
            onSlideChange: function (slide_index) {
                var window_width = $(window).width();
                if (parseInt(window_width) > 600) {
                    var slide = $('.slideshow__item')[slide_index]
                    var image = $(slide.firstChild)
